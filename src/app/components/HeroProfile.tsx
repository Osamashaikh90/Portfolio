...
const HeroProfile = () => {
 const handleHireButtonClick = () => {
  ...
  return (
    <>
      <div className='col-span-1 sm:col-start-1 sm:col-end-3'>
        <div className='flex items-center gap-5 sm:block'>
        <div className='profile mt-11 mb-2 relative'> <Image
                            className='sm:hidden select-none rounded-[50%]'
         src="https://res.cloudinary.com/dwohsn29d/image/upload/v1696745241/Portfolio/images/uls26z6z64kqaxccvnlh.jpg"width={100}
        height={100}
        alt="profile" />
        
 <Image
                            className='hidden sm:block select-none rounded-[50%]'
         src="https://res.cloudinary.com/dwohsn29d/image/upload/v1696745241/Portfolio/images/uls26z6z64kqaxccvnlh.jpg"
        width={540}
        height={540}
        alt="profile" />
        <div data-tooltip-id="my-tooltip-1" className="hidden sm:block cursor-pointer emoji absolute right-0 bottom-9 rounded-full bg-[#010409] border border-[#21262D] w-10 h-10 p-[10px] text-sm">😄</div>
      </div>
        <div className='mt-5 text-2xl sm:text-[26px] font-bold sm:font-semibold font-sans text-[#e6edf3]'>Osama Shaikh<span className='text-[#7d8590] font-normal text-xl'> . </span><span className='text-[#7d8590] font-normal text-lg sm:text-xl'>he/him</span></div></div>
       <div  className=" sm:hidden cursor-pointer emoji bg-[##0d1117] border border-[#21262D] w-full rounded-lg mt-5 px-2 py-[6px] text-sm">😄 <span className='text-[#e6edf3]'>learning!</span></div>
